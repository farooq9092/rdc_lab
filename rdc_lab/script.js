...
const initPortal = () => {
    ...
    authForm.addEventListener('submit', async (e) => {
        ...
        try {
            const response = await fetch(`/api/reports/${caseId}?cnic=${cnic}`);
            ...
            if (data.status === 'Final') {
                ...
                action.innerHTML = `<a href="/api/reports/download/${caseId}?cnic=${cnic}" class="btn btn-secondary" style="width:100%; display:inline-block; margin-top:10px" target="_blank"><i class="fas fa-file-pdf"></i> Download Official PDF Report</a>`;
            } else {
